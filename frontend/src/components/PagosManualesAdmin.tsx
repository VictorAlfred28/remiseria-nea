  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-nowrap gap-2 overflow-x-auto scrollbar-thin w-full pb-1 bg-zinc-900 border border-zinc-800 rounded-xl p-1 scroll-smooth" style={{ WebkitOverflowScrolling: 'touch' }}>
            <button 
                onClick={() => setFilter('PENDIENTE')} 
                className={`flex-shrink-0 whitespace-nowrap px-4 py-2 rounded-lg text-sm font-bold transition-all ${filter === 'PENDIENTE' ? 'bg-yellow-500/20 text-yellow-500' : 'text-zinc-500 hover:text-white'}`}
            >
                Pendientes
            </button>
            <button 
                onClick={() => setFilter('APROBADO')} 
                className={`flex-shrink-0 whitespace-nowrap px-4 py-2 rounded-lg text-sm font-bold transition-all ${filter === 'APROBADO' ? 'bg-green-500/20 text-green-500' : 'text-zinc-500 hover:text-white'}`}
            >
                Aprobados
            </button>
            <button 
                onClick={() => setFilter('RECHAZADO')} 
                className={`flex-shrink-0 whitespace-nowrap px-4 py-2 rounded-lg text-sm font-bold transition-all ${filter === 'RECHAZADO' ? 'bg-red-500/20 text-red-500' : 'text-zinc-500 hover:text-white'}`}
            >
                Rechazados
            </button>
        </div>
