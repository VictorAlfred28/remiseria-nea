                    ...
                    {(activeTariff || fixedDestinations?.length > 0) && (
                        <div className="bg-zinc-900 border border-zinc-800 p-4 sm:p-6 rounded-2xl flex flex-col gap-6 shadow-xl">
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-center">
                                <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-700/50 flex flex-col justify-center hover:border-blue-500/30 transition-colors">
                                    <p className="text-zinc-500 text-[10px] sm:text-xs uppercase font-bold tracking-widest mb-1">Tarifa Mínima</p>
                                    <p className="text-cyan-400 font-black text-xl sm:text-2xl">${(activeTariff?.base_fare || 2500).toLocaleString('es-AR')}</p>
                                </div>
                                <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-700/50 flex flex-col justify-center hover:border-blue-500/30 transition-colors">
                                    ...
                                    <p className="text-cyan-400 font-black text-xl sm:text-2xl">${(activeTariff?.per_fraction_price || 175).toLocaleString('es-AR')}</p>
                                </div>
                                <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-700/50 flex flex-col justify-center hover:border-blue-500/30 transition-colors">
                                    <p className="text-zinc-500 text-[10px] sm:text-xs uppercase font-bold tracking-widest mb-1">Espera 10 Min.</p>
                                    <p className="text-cyan-400 font-black text-xl sm:text-2xl">${(activeTariff?.wait_block_price || 2500).toLocaleString('es-AR')}</p>
                                </div>
                                <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-700/50 flex flex-col justify-center hover:border-blue-500/30 transition-colors">
                                    <p className="text-zinc-500 text-[10px] sm:text-xs uppercase font-bold tracking-widest mb-1">Costo de Baúl</p>
                                    <p className="text-cyan-400 font-black text-xl sm:text-2xl">${(activeTariff?.trunk_price || 2500).toLocaleString('es-AR')}</p>
                                </div>
                            </div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 max-h-[500px] overflow-y-auto scroll-smooth pr-1">
                                    {(() => {
                                        ...
                                        for (let d = 1.0; d <= 20.0; d += fraction) {
                                            ...
                                            table.push(
                                                <div key={d.toFixed(1)} className="flex items-center justify-between bg-zinc-900 px-3 py-2.5 rounded-lg border border-zinc-800 hover:border-blue-500/50 hover:bg-blue-500/5 transition-all group">
                                                    ...
                                                    <span className="text-cyan-400 font-black text-sm">${price.toLocaleString('es-AR')}</span>
                                                </div>
                                            );
                                        }
                                        return table;
