                    ...
                    <div className="bg-gradient-to-b from-blue-950/40 to-zinc-900 border-2 border-blue-500/40 p-6 rounded-2xl shadow-2xl animate-in zoom-in-95 duration-300">
                        ...
                        <div className="space-y-4 mb-8">
                            <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                                ...
                                {viajeActivo.origen?.observaciones && (
                                    <p className="text-amber-400 text-sm mt-2 font-medium bg-amber-500/10 px-3 py-1.5 rounded-lg border border-amber-500/20">
                                        📝 {viajeActivo.origen.observaciones}
                                    </p>
                                )}
                                {viajeActivo.origen?.referencia && (
                                    <p className="text-blue-400 text-sm mt-1 font-medium bg-blue-500/10 px-3 py-1.5 rounded-lg border border-blue-500/20">
                                        📍 Ref: {viajeActivo.origen.referencia}
                                    </p>
                                )}
                            </div>
                            <div className="bg-zinc-950 p-4 rounded-xl border border-zinc-800">
                            ...
                            <LocalWaitTimer isActive={normalizeEstado(viajeActivo.estado) === 'EN_PUERTA'} />
                            ...
                            <div className="bg-blue-950/20 p-4 rounded-xl border border-blue-500/20 text-center">
                                ...
                                {normalizeEstado(viajeActivo.estado) === 'INICIADO' && (
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            <div className="grid grid-cols-2 gap-3 mt-2">
                               {normalizeEstado(viajeActivo.estado) === 'ACEPTADO' ? (
                                   <button 
                               ...
                               ) : normalizeEstado(viajeActivo.estado) === 'EN_PUERTA' ? (
                                   <button 
                            ...
                            <button 
                                disabled={normalizeEstado(viajeActivo.estado) === 'ACEPTADO' || normalizeEstado(viajeActivo.estado) === 'EN_PUERTA'}
                                onClick={handleFinalizarViaje}
                                className={`w-full py-4 text-white font-bold rounded-2xl flex items-center justify-center gap-2 transition-all ${
                                    (normalizeEstado(viajeActivo.estado) === 'ACEPTADO' || normalizeEstado(viajeActivo.estado) === 'EN_PUERTA')
                                ...
                                return (
                                    <div key={viaje.id} className={`bg-zinc-900 border ${distancia_destino !== null && parseFloat(distancia_destino) < 2 ? 'border-blue-500/40' : 'border-zinc-800'} p-5 rounded-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 transition-all hover:border-zinc-700 animate-in slide-in-from-top-4 fade-in duration-300`}>
                                        <div className="w-full">
                                            ...
                                            {(viaje.origen?.observaciones || viaje.origen?.referencia) && (
                                                <div className="mt-2 text-xs font-medium space-y-1 bg-black/20 p-2 rounded-lg border border-white/5">
                                                    {viaje.origen?.observaciones && <p className="text-amber-400/90">📝 {viaje.origen.observaciones}</p>}
                                                    {viaje.origen?.referencia && <p className="text-blue-400/90">📍 {viaje.origen.referencia}</p>}
                                                </div>
                                            )}
