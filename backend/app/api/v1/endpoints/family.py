...
def delete_family_zone(zone_id: str, claims: Dict[str, Any] = Depends(get_current_user)):
    ...
    # Verificar que la zona pertenece al grupo familiar del usuario
    zone = supabase.table("family_zones").select("grupo_id").eq("id", zone_id).execute()
    if not zone.data:
        raise HTTPException(status_code=404, detail="Zona no encontrada.")
    
    grupo_id = zone.data[0]["grupo_id"]
    group = supabase.table("grupos_familiares").select("id").eq("id", grupo_id).eq("tutor_user_id", tutor_id).execute()
    if not group.data:
