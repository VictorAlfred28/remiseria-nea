...
def has_role(claims: Dict[str, Any], role: str) -> bool:
    """
    Comprueba si el usuario posee un rol dado verificando AMBAS fuentes (insensible a mayúsculas):
    ...
    claim_rol = claims.get("rol")
    if claim_rol and str(claim_rol).lower() == target:
        return True
        
    # 2. Lista de roles extendida (puede estar vacía si aún no hay registros)
    for r in claims.get("roles", []):
        if r and str(r).lower() == target:
            return True
...
def verify_token(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            res = supabase.table("usuarios").select("rol, organizacion_id, requiere_cambio_password").eq("id", user_id).execute()
        except Exception as query_error:
            # Fallback en caso de que la migración SQL aún no se haya ejecutado
            if "does not exist" in str(query_error):
                res = supabase.table("usuarios").select("rol, organizacion_id").eq("id", user_id).execute()
                # Simular que no requiere cambio
                for item in res.data:
                    item["requiere_cambio_password"] = False
            else:
                raise
        ...
        claims = {
            ...
            "requiere_cambio_password": res.data[0].get("requiere_cambio_password", False),
        }
...
def require_password_changed(claims: Dict[str, Any] = Depends(get_current_user)) -> Dict[str, Any]:
    """Guard que exige que el usuario haya cambiado su contraseña inicial."""
    if claims.get("requiere_cambio_password"):
        raise HTTPException(status_code=403, detail="Debe cambiar su contraseña inicial para acceder a este recurso.")
    return claims


def get_current_admin(claims: Dict[str, Any] = Depends(require_password_changed)) -> Dict[str, Any]:
...
def get_current_chofer(claims: Dict[str, Any] = Depends(require_password_changed)) -> Dict[str, Any]:
...
def get_current_cliente(claims: Dict[str, Any] = Depends(require_password_changed)) -> Dict[str, Any]:
    """Guard exclusivo para pasajeros. Acepta 'cliente', 'pasajero', 'titular', 'admin', 'superadmin'."""
    if not (has_role(claims, "cliente") or has_role(claims, "pasajero") or has_role(claims, "titular") or has_role(claims, "admin") or has_role(claims, "superadmin")):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No authorized as cliente")
    return claims


def get_current_titular(claims: Dict[str, Any] = Depends(require_password_changed)) -> Dict[str, Any]:
