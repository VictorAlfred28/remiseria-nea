...
class TariffConfigCreate(BaseModel):
    ...
    fraction_km: float
    # Campos v2.0
    wait_free_minutes: int = 5
    wait_block_minutes: int = 10
    wait_block_price: float = 2500.0
    wait_price_per_minute: float = 250.0
    trunk_price: float = 2500.0
...
class TariffConfigResponse(BaseModel):
    ...
    fraction_km: float
    is_active: bool
    # Campos v2.0 opcionales para evitar crashes
    wait_free_minutes: int = 5
    wait_block_minutes: int = 10
    wait_block_price: float = 2500.0
    wait_price_per_minute: float = 250.0
...
def get_active_tariff(claims: Dict[str, Any] = Depends(get_current_user)):
...
def create_tariff(data: TariffConfigCreate, claims: Dict[str, Any] = Depends(get_current_admin)):
    ...
    new_tariff = {
        ...
        "is_active": True,
        # Campos v2.0
        "wait_free_minutes": data.wait_free_minutes,
        "wait_block_minutes": data.wait_block_minutes,
        "wait_block_price": data.wait_block_price,
        "wait_price_per_minute": data.wait_price_per_minute,
        "trunk_price": data.trunk_price,
        "dynamic_multiplier": data.dynamic_multiplier
    }
...
def get_destinations(claims: Dict[str, Any] = Depends(get_current_user)):
