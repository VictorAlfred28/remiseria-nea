...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  logout: async () => {
    ...
    set({ user: null, role: null, roles: [], orgId: null, requiere_cambio_password: false, isLoading: false });
  },
  ...
  checkSession: async () => {
    try {
      ...
      if (error || !session) {
        set({ user: null, role: null, roles: [], requiere_cambio_password: false, isLoading: false });
        return;
      }

      // Save token for axios using secure localStorage (Capacitor compatible)
      localStorage.setItem('sb-access-token', session.access_token);
      sessionStorage.setItem('sb-access-token', session.access_token);

      // 1. Perfil base con rol legado
      let userDataResult = await supabase
        .from('usuarios')
        .select('rol, organizacion_id, requiere_cambio_password')
        .eq('id', session.user.id)
        .single();

      // Fallback si la migración de columna requiere_cambio_password aún no se aplicó
      if (userDataResult.error && userDataResult.error.code === '42703') {
        ...
        if (userDataResult.data) {
          userDataResult.data.requiere_cambio_password = false;
        }
      }
      ...
      set({
        ...
        requiere_cambio_password: userData.requiere_cambio_password || false,
        isLoading: false,
      });
