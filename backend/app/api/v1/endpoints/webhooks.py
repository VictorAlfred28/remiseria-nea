...
async def evolution_webhook(request: Request, background_tasks: BackgroundTasks, event_path: str = ""):
    """
    ...
    try:
        ...
        if is_me:
            ...
            if "conversation" in msg_obj:
                texto_mio = msg_obj["conversation"]
            elif "extendedTextMessage" in msg_obj:
                texto_mio = msg_obj["extendedTextMessage"].get("text", "")
                
            phone_number = remote_jid.split("@")[0]
            texto_mio_lower = texto_mio.strip().lower()
            
            if texto_mio_lower in ["/cerrar", "/finalizar"]:
                from app.db.supabase import supabase
                from app.core.evolution import send_whatsapp_message
                supabase.table("chat_sessions").update({"operador_humano": False, "estado": "negociando"}).eq("telefono", phone_number).execute()
                background_tasks.add_task(send_whatsapp_message, instance, phone_number, "🤖 El asesor ha finalizado la atención. El asistente virtual vuelve a estar activo.")
                return {"status": "operator_closed"}
            else:
                from datetime import datetime, timezone
                from app.db.supabase import supabase
                supabase.table("chat_sessions").update({"actualizado_en": datetime.now(timezone.utc).isoformat()}).eq("telefono", phone_number).execute()
        ...
        if row_id and row_id.startswith("rate_"):
             ...
             if len(partes) >= 3:
                 ...
                 phone_number = remote_jid.split("@")[0]
                 mensaje_gracias = (
                     "🚖 Gracias por elegir UBI Traslados.\n\n"
                     f"¡Gracias por tu calificación de {puntaje} estrellas! Trabajamos para brindarte el mejor servicio.\n\n"
                     "Además te invitamos a seguirnos en Facebook:\nUBI Traslados"
                 )
                 background_tasks.add_task(send_whatsapp_message, instance, phone_number, mensaje_gracias)
        ...
        if es_audio:
            ...
            texto_recibido = f"📍 [UBICACIÓN GPS] Lat: {lat}, Lng: {lng}"
            logger.info(f"Ubicación GPS recibida: {texto_recibido}")
