...
import {
  Mail, ArrowLeft, CheckCircle2, Car, ShieldCheck,
} from 'lucide-react';
...
export default function Login() {
      ...
      if (data.session) {
        ...
        if (profileError) {
          if (profileError.code === 'PGRST116') {
            throw new Error('No se encontró un perfil de usuario asociado a esta cuenta. Contactá al soporte.');
          }
        ...
        const isPrimarySuperAdmin =
          userData?.rol === 'superadmin' && data.session.user.email === 'agentech.nea@gmail.com';
        ...
        if (redirectUrl) {
          ...
          if (rol === 'admin' || rol === 'superadmin') navigate('/admin');
          else if (rol === 'chofer') navigate('/chofer');
          else if (rol === 'comercio') navigate('/comercio');
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!identificador) {
      setErrorMsg('Por favor, ingresá tu correo electrónico o DNI.');
      return;
    }
    setLoading(true);
    setErrorMsg('');
    ...
    >
      <div className="w-full flex flex-col h-full" style={{ maxWidth: '600px' }}>
        ...
        >
          ...
          <div className="w-full max-w-[430px] mx-auto px-5 pt-6 pb-5">
            ...
            {errorMsg && (
              <div
                id="login-error"
                role="alert"
                className="mb-4 text-xs px-3 py-2.5 rounded-xl bg-red-500/10 border border-red-500/30 text-red-300"
              >
            ...
            {/* ── Reset success ── */}
            {resetSuccess ? (
              <div className="flex flex-col items-center text-center py-6 px-4 bg-green-500/5 rounded-2xl border border-green-500/20">
                ...
                <button
                  ...
                  style={{ width: 'auto' }}
                  className="text-xs text-blue-400 underline font-medium hover:text-blue-300 transition-colors"
                >
            ...
            /* ── Reset form ── */
              ...
              <form onSubmit={handleResetPassword} className="space-y-4 animate-in slide-in-from-right-4 fade-in">
                <div className="relative">
                  <span className="absolute left-0 top-0 bottom-0 flex items-center pl-3.5 pointer-events-none" aria-hidden="true">
            ...
            /* ── Login form ── */
              ...
              <form onSubmit={handleLogin} className="space-y-3.5 animate-in slide-in-from-left-4 fade-in">
                {/* Identificador */}
                <div>
                ...
                <AuthButton
                  ...
                  variant="white"
                  id="login-submit-btn"
                  className="mt-1 !py-2.5 !text-base !font-black !text-[#071B4D]"
                >
                  Ingresar
                </AuthButton>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <button
                    ...
                    onClick={() => window.open('https://www.facebook.com/people/UBITraslados/61566631841093/', '_blank')}
                    style={{ width: '100%' }}
                    className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs font-semibold hover:bg-white/10 active:scale-[0.97] transition"
                    aria-label="Visitar Facebook de UBI Traslados"
                  >
                  ...
                  <button
                    ...
                    style={{ width: '100%' }}
                    className="flex items-center justify-center gap-1.5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs font-semibold hover:bg-white/10 active:scale-[0.97] transition"
                    aria-label="Visitar Instagram de UBI Traslados"
                  >
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="url(#ig-grad-login)" aria-hidden="true">
                      <defs>
                        <linearGradient id="ig-grad-login" x1="0%" y1="100%" x2="100%" y2="0%">
              ...
              <div className="mt-5 text-center space-y-2">
                ...
                <div className="flex items-center justify-center gap-4 pt-1">
                  <div className="flex items-center gap-1 text-[#4a6fa5]">
                    ...
                    <span className="text-[10px]">Viajes seguros</span>
                  </div>
                  <div className="flex items-center gap-1 text-[#4a6fa5]">
                    <ShieldCheck size={12} aria-hidden="true" />
                    <span className="text-[10px]">Choferes verificados</span>
                  </div>
                </div>
              </div>
            )}
