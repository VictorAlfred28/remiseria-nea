...
import ValidacionPublica from "./pages/ValidacionPublica";
...
function ProtectedRoute({ children, allowedRoles }: { children: ReactNode, allowedRoles?: string[] }) {
  const { user, role, isLoading, isRecoveringPassword, requiere_cambio_password } = useAuthStore();
  ...
  if (requiere_cambio_password && location.pathname !== '/cambiar-password') {
          ...
          <nav className="w-full glass-panel !border-l-0 !border-r-0 !border-t-0 p-4 shadow-[0_4px_30px_rgba(0,0,0,0.5)] flex justify-between items-center border-b border-[rgba(255,255,255,0.08)] sticky top-0 z-50 safe-pt">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                title="Cerrar Sesión"
                className="flex items-center justify-center w-9 h-9 text-zinc-500 hover:text-red-400 transition-all duration-300 bg-white/5 hover:bg-red-500/10 rounded-full border border-white/5 hover:border-red-500/30 hover:shadow-[0_0_12px_rgba(239,68,68,0.2)] active:scale-90"
              >
                <LogOut size={16} />
              </button>
            </div>
          ...
          <Routes>
              ...
              isRecoveringPassword ? (
                ...
                <div className="flex items-center justify-center p-10 h-[80vh]">
                  {isLoading ? (
                    ...
                    useAuthStore.getState().requiere_cambio_password ? <Navigate to="/cambiar-password" replace /> :
            ...
            <Route path="/admin" element={
              <ProtectedRoute allowedRoles={['admin', 'superadmin']}>
                <div className="w-full px-2 sm:px-4 py-4"><AdminDashboard /></div>
              </ProtectedRoute>
            } />
            ...
            <Route path="/chofer/*" element={
              <ProtectedRoute allowedRoles={['chofer']}>
                <div className="w-full px-2 sm:px-4 py-4"><ChoferDashboard /></div>
              </ProtectedRoute>
            } />

            <Route path="/cliente" element={
              <ProtectedRoute allowedRoles={['cliente', 'titular']}>
                <div className="w-full px-2 sm:px-4 py-4"><ClienteDashboard /></div>
              </ProtectedRoute>
            } />

            <Route path="/comercio" element={
              <ProtectedRoute allowedRoles={['comercio']}>
                <div className="w-full px-2 sm:px-4 py-4"><ComercioDashboard /></div>
              </ProtectedRoute>
            } />
            
            <Route path="/promocion/:id" element={
              <ProtectedRoute>
                <div className="w-full px-2 sm:px-4 py-4"><PromocionDetalle /></div>
              </ProtectedRoute>
            } />
