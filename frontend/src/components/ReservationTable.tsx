    ...
    return (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-300">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
               ...
               <div className="flex flex-nowrap gap-1 overflow-x-auto scrollbar-thin max-w-full w-full bg-zinc-900 border border-zinc-800 rounded-xl p-1 pb-1" style={{ WebkitOverflowScrolling: 'touch' }}>
                   {['todas', 'pendiente', 'confirmada', 'cancelada'].map(f => (
                       <button
                         key={f}
                         onClick={() => setFilter(f)}
                         className={`flex-shrink-0 whitespace-nowrap max-w-full w-auto px-4 py-2 rounded-lg text-xs font-bold uppercase transition-all ${filter === f ? 'bg-blue-600 text-white shadow' : 'text-zinc-500 hover:text-white hover:bg-zinc-800/50'}`}
                       >
                           {f}
                       </button>
                   ))}
            ...
            <div className="bg-zinc-900/50 border border-zinc-800 rounded-2xl overflow-x-auto shadow-xl w-full">
