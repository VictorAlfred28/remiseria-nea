...
export default function TramitesTab({ vehiculos, choferId }: { vehiculos?: Vehicle[]; choferId?: string }) {
  ...
  const handleDelete = async (t: Tramite) => {
    if (!confirm("¿Eliminar este trámite?")) return;
    try {
      await api.delete(`/tramites/${t.id}`);
      if (t.archivo_url) {
        const oldPath = t.archivo_url.split('/').pop();
        if (oldPath) await supabase.storage.from('tramites_vehiculares').remove([`tramites/${oldPath}`]);
      }
      fetchTramites();
    } catch (e) {
      console.error("Error eliminando trámite", e);
    }
  };
            ...
            return (
              <div key={t.id} className={`bg-white border rounded-2xl p-5 flex flex-col gap-4 transition-all hover:shadow-md
                ...
                <div className="flex items-start justify-between">
                  ...
                  <button onClick={() => handleDelete(t)} className="text-zinc-300 hover:text-red-500 transition-colors">
                    <Trash2 size={15} />
                  </button>
                </div>
