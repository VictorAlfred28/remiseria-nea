            ...
            {!verDetalle ? (
                <>
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-2">
                        <div>
                            <h2 className="text-2xl font-black text-white flex items-center gap-2"><Building className="text-blue-500 shrink-0" size={24} /> Empresas Adheridas</h2>
                            <p className="text-zinc-500 text-sm mt-1">Administra las compañías corporativas, descuentos y empleados autorizados.</p>
                        </div>
                        <button onClick={() => { setId(""); setNombre(""); setCuit(""); setShowForm(true); }} className="w-full md:w-auto px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-bold rounded-xl flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-500/25 shrink-0">
                        ...
                        <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl">
                            ...
                            <form onSubmit={handleSaveEmpresa} className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                ...
                                <div className="flex items-end">
                                    <button disabled={loading} type="submit" className="w-full max-w-full min-h-[52px] px-4 py-3 text-base font-semibold text-center overflow-hidden whitespace-normal break-words shrink-0 bg-emerald-600 text-white rounded-xl hover:bg-emerald-500 flex items-center justify-center">
                                        {loading ? '...' : 'Guardar'}
                                    </button>
                                </div>
                            </form>
                    ...
                    {loading && !showForm ? <div className="p-12 text-center"><Loader2 className="animate-spin inline-block text-blue-500" /></div> : (
                        <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden overflow-x-auto w-full">
...
function EmpresaDetalle({ empresa, goBack, fetchList }: any) {
    ...
    return (
        <div className="animate-in fade-in zoom-in-95 duration-300">
            ...
            <div className="flex flex-nowrap gap-4 mb-6 border-b border-zinc-800 pb-2 overflow-x-auto scrollbar-thin w-full" style={{ WebkitOverflowScrolling: 'touch' }}>
                <button onClick={() => setActiveTab('usuarios')} className={`whitespace-nowrap flex-shrink-0 pb-2 font-bold px-2 border-b-2 flex items-center gap-2 ${activeTab==='usuarios' ? 'border-blue-500 text-blue-400' : 'border-transparent text-zinc-500'}`}><Users size={16}/> Empleados Asignados</button>
                <button onClick={() => setActiveTab('beneficios')} className={`whitespace-nowrap flex-shrink-0 pb-2 font-bold px-2 border-b-2 flex items-center gap-2 ${activeTab==='beneficios' ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-zinc-500'}`}><Percent size={16}/> Descuentos Autorizados</button>
                <button onClick={() => setActiveTab('cuenta')} className={`whitespace-nowrap flex-shrink-0 pb-2 font-bold px-2 border-b-2 flex items-center gap-2 ${activeTab==='cuenta' ? 'border-amber-500 text-amber-400' : 'border-transparent text-zinc-500'}`}><CreditCard size={16}/> Cuenta Corriente</button>
            </div>
            ...
            {activeTab === 'cuenta' && (
                <div className="space-y-6">
                    ...
                    <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
                        ...
                        <div className="max-h-[70vh] overflow-y-auto overflow-x-auto w-full">
