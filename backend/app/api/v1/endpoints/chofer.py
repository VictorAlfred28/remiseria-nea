...
def iniciar_viaje(viaje_id: str, background_tasks: BackgroundTasks, claims: Dict[str, Any] = Depends(get_current_chofer)):
    ...
    """
    chofer_user_id = claims.get("sub")
    v_resp = supabase.table("viajes").select("id, origen, choferes!inner(usuario_id)").eq("id", viaje_id).execute()
    if not v_resp.data:
        raise HTTPException(status_code=404, detail="Viaje no encontrado.")
    ...
    # 2. Enviar WhatsApp de inicio de viaje
    viaje = v_resp.data[0]
    pasajero_phone = viaje.get("origen", {}).get("cliente_telefono")
    ai_instance = viaje.get("origen", {}).get("ai_instance", "viajesnea")
    
    if pasajero_phone:
        msg = "🚀 *¡Tu viaje ha comenzado!*\n\nPonte cómodo y disfruta del recorrido con nosotros. Si necesitas algo, no dudes en decirle a tu chofer."
        background_tasks.add_task(send_whatsapp_message, ai_instance, pasajero_phone, msg)
...
def notificar_finalizacion(viaje_id: str, background_tasks: BackgroundTasks, claims: Dict[str, Any] = Depends(get_current_chofer)):
    # DESAPROBADO: Usado previamente para enviar doble notificación.
    # Ahora la notificación final está centralizada en finalizar_viaje_transaccional / notificar_finalizacion_v2
    # para evitar mensajes duplicados de WPP.
    return {"message": "Notificación delegada al endpoint finalizar_viaje. Endpoint deprecado.", "sent": False}
...
async def registrar_pago_chofer(
    ...
    import uuid
    # 1. Leer el archivo y validar
    file_bytes = await comprobante.read()
    file_size_mb = len(file_bytes) / (1024 * 1024)
    file_ext = comprobante.filename.split(".")[-1].lower() if "." in comprobante.filename else "jpg"
    mime_type = comprobante.content_type
    ...
    if mime_type not in allowed_mimes:
        raise HTTPException(status_code=400, detail=f"Formato no permitido: {mime_type}. Usa JPG, PNG, WEBP o PDF.")
    ...
    if mime_type == "application/pdf" and file_bytes[:4] != b"%PDF":
        raise HTTPException(status_code=400, detail="El archivo PDF está corrupto o es inválido.")

    file_name = f"{c_id}/{uuid.uuid4()}.{file_ext}"

    # 2. Subir a Supabase Storage (bucket 'comprobantes') con Cache-Control
    try:
         # Usamos upsert para evitar fallos si el UUID choca (casi imposible) e indicamos Cache-Control
         supabase.storage.from_("comprobantes").upload(file_name, file_bytes, {"cache-control": "public, max-age=31536000", "content-type": mime_type})
    ...
    # 4. Insertar en BD con ROLLBACK automático si falla
    org_id = claims.get("organizacion_id")
    try:
        ...
        # ROLLBACK: Eliminar archivo subido si falla la DB para evitar huérfanos inmediatos
        try:
             supabase.storage.from_("comprobantes").remove([file_name])
