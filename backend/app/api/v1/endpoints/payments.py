...
def acreditar_pago(chofer_id: str, monto: float, mp_payment_id: str):
    ...
    try:
        ...
        # 2. Actualizar saldo atómicamente usando RPC para evitar race conditions
        try:
            supabase.rpc("incrementar_saldo_chofer", {"p_chofer_id": chofer_id, "p_monto": monto}).execute()
        except Exception:
            # Fallback si la función RPC no existe: update directo (menos seguro pero funcional)
...
def admin_manual_charge(data: AdminChargeRequest, claims: Dict[str, Any] = Depends(get_current_admin)):
    ...
    try:
        supabase.rpc("incrementar_saldo_chofer", {"p_chofer_id": chofer_id, "p_monto": monto}).execute()
        # Leer saldo actualizado para retornarlo
        chofer_req = supabase.table("choferes").select("saldo").eq("id", chofer_id).execute()
        nuevo_saldo = chofer_req.data[0].get("saldo", 0) if chofer_req.data else 0
    except Exception:
