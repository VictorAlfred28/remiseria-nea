...
def create_chofer(data: ChoferRegistroCompleto, claims: Dict[str, Any] = Depends(get_current_admin)):
    ...
    # 3. Actualizar Perfil Usuario (ya creado por trigger auth)
    try:
        ...
        try:
            ...
            pass
        raise HTTPException(status_code=500, detail=f"Falla actualizando perfil de usuario: {str(e)}")
...
def get_admin_dashboard_stats(claims: Dict[str, Any] = Depends(get_current_admin)):
    """
    Obtiene las estadísticas resumidas para el dashboard de admin.
    """
    org_id = claims.get("organizacion_id")
    
    # Choferes
    c_resp = supabase.table("choferes").select("id", count="exact").eq("organizacion_id", org_id).execute()
    choferes_count = c_resp.count if c_resp.count else 0
    
    # Viajes Totales
    v_resp = supabase.table("viajes").select("id", count="exact").eq("organizacion_id", org_id).execute()
    viajes_count = v_resp.count if v_resp.count else 0
    
    # Viajes Hoy (Filtro por fecha en Python o BD)
    hoy = datetime.now(timezone.utc).date()
    v_hoy_resp = supabase.table("viajes").select("id", count="exact").eq("organizacion_id", org_id).gte("creado_en", hoy.isoformat()).execute()
    viajes_hoy = v_hoy_resp.count if v_hoy_resp.count else 0
    
    return {
        "choferes_activos": choferes_count,
        "viajes_totales": viajes_count,
        "viajes_hoy": viajes_hoy,
        "recaudacion_estimada": 0  # Reemplazar con lógica real
    }

@router.get("/storage-health")
def get_storage_health(claims: Dict[str, Any] = Depends(get_current_admin)):
    """
    Métricas de salud del Storage y sistema de cleanup.
    """
    # Total de archivos
    f_resp = supabase.table("file_references").select("id, file_size, is_active", count="exact").execute()
    total_archivos = f_resp.count if f_resp.count else 0
    archivos_data = f_resp.data if f_resp.data else []
    
    # Tamaños
    espacio_usado_bytes = sum(f.get("file_size", 0) or 0 for f in archivos_data if f.get("is_active"))
    
    # Huérfanos/Inactivos
    huerfanos = len([f for f in archivos_data if not f.get("is_active")])
    
    # Liberado (desde logs)
    l_resp = supabase.table("storage_cleanup_logs").select("tamano_liberado").eq("estado", "ELIMINADO").execute()
    logs_data = l_resp.data if l_resp.data else []
    espacio_liberado_bytes = sum(l.get("tamano_liberado", 0) or 0 for l in logs_data)
    
    return {
        "total_archivos_registrados": total_archivos,
        "archivos_activos": total_archivos - huerfanos,
        "huerfanos_pendientes_borrado": huerfanos,
        "espacio_usado_mb": round(espacio_usado_bytes / (1024 * 1024), 2),
        "espacio_liberado_historico_mb": round(espacio_liberado_bytes / (1024 * 1024), 2),
...
def get_choferes(claims: Dict[str, Any] = Depends(get_current_admin)):
    """
    Obtener todos los choferes con detalles de usuario (nombre, email, telefono)
    """
    org_id = claims.get("organizacion_id")
...
class TitularCreate(BaseModel):
    ...
    email: EmailStr
    telefono: str
    dni: str  # DNI obligatorio del titular
...
def create_titular(
    ...
    FLUJO CORRECTO (compatible con trigger handle_new_user):
    1. Validaciones previas (email único, DNI único)
    2. Crear en Supabase Auth → el trigger crea automáticamente la fila en public.usuarios
    3. UPDATE de esa fila (NO INSERT) para completar los datos del titular
    4. INSERT en user_roles para multi-rol
    5. Notificación WhatsApp en background
    """
    org_id = claims.get("organizacion_id")

    # ── VALIDACIÓN 1: DNI solo numérico ────────────────────────────────────────
    if not data.dni.strip().isdigit():
        raise HTTPException(
            status_code=422,
            detail="El DNI debe contener únicamente dígitos numéricos."
        )

    # ── VALIDACIÓN 2: Email duplicado ──────────────────────────────────────────
    email_check = supabase.table("usuarios").select("id").eq("email", data.email).execute()
    if email_check.data:
        raise HTTPException(
            status_code=409,
    ...
    # ── VALIDACIÓN 3: DNI duplicado ────────────────────────────────────────────
    # Verificar en usuarios y en choferes (ambas tablas, ambos roles)
    dni_strip = data.dni.strip()

    # 3a. Verificar en public.usuarios (cubre titulares ya registrados)
    try:
        dni_check_usr = supabase.table("usuarios").select("id").eq("dni", dni_strip).execute()
        if dni_check_usr.data:
            raise HTTPException(
                status_code=409,
                detail="Ya existe un usuario registrado con ese número de DNI."
            )
    except HTTPException:
        raise
    except Exception:
        pass  # Si la columna dni no existe aún en usuarios (migración pendiente), continuar

    # 3b. Verificar en public.choferes (cubre choferes con ese DNI)
    try:
        dni_check_ch = supabase.table("choferes").select("id").eq("dni", dni_strip).execute()
        if dni_check_ch.data:
            raise HTTPException(
                status_code=409,
                detail="Ya existe un conductor registrado con ese número de DNI."
            )
    except HTTPException:
        raise
    except Exception:
        pass  # Si la columna no existe en choferes, continuar

    # ── PASO 1: Generar contraseña inicial ────────────────────────────────────
    password = "UBI2026!"

    # ── PASO 2: Crear en Supabase Auth ─────────────────────────────────────────
    # IMPORTANTE: Al crear el usuario en Auth, el trigger `handle_new_user`
    # se dispara automáticamente e inserta una fila en public.usuarios.
    # Por eso el siguiente paso debe ser UPDATE, NO INSERT.
    user_id = None
    try:
        auth_res = supabase.auth.admin.create_user({
            "email": data.email,
        ...
        raise HTTPException(
            status_code=400,
            detail="No se pudo crear el acceso. Verificá los datos e intentá nuevamente."
        )

    # ── PASO 3: UPDATE del perfil creado por el trigger ────────────────────────
    # El trigger ya insertó la fila. Actualizamos para completar los datos del titular.
    try:
        update_resp = supabase.table("usuarios").update({
            ...
            "rol": "titular",
            "activo": True,
            "dni": dni_strip,  # Campo DNI persistido desde la migración
            "requiere_cambio_password": True
        }).eq("id", user_id).execute()

        if not update_resp.data:
            raise Exception("No se actualizó ningún registro. El trigger puede no haber ejecutado aún.")

    except HTTPException:
        raise
    except Exception as e:
        # Rollback: borrar usuario de Auth para no dejar registros huérfanos
        try:
            supabase.auth.admin.delete_user(user_id)
        except Exception:
            pass
        raise HTTPException(
            status_code=500,
            detail="Error completando el registro del titular. El acceso fue revertido."
        )

    # ── PASO 4: Registrar en user_roles para soporte multi-rol ────────────────
    try:
        ...
        # No crítico: el sistema funciona con el campo rol de usuarios
        pass


    # ── PASO 6: Notificación WhatsApp en background ───────────────────────────
