...
import { Car, Mail, Loader2, ArrowLeft, User, Phone, CheckCircle2, ChevronRight, UploadCloud, MapPin, X } from 'lucide-react';
...
import { optimizeUpload } from '../utils/uploadOptimizer';
...
export default function RegisterChofer() {
  ...
  const nextStep = () => {
    ...
    if (step === 4 && (!frenteFile || !dorsoFile)) {
      setErrorMsg('Debes subir los documentos requeridos (Frente y Dorso de la Licencia).');
      return;
    }
    setStep(s => s + 1);
  };
  ...
  const uploadFile = async (rawFile: File, folderId: string, prefix: string) => {
    setIsOptimizing(true);
    let fileToUpload = rawFile;
    try {
       const { file } = await optimizeUpload(rawFile, 'general');
       fileToUpload = file;
    } catch(e: any) {
    ...
    const fileName = `${folderId}/${prefix}-${uniqueId}.${fileExt}`;
    // Assuming 'documentos_choferes' is set to public or authenticated
    const { data, error } = await supabase.storage.from('documentos_choferes').upload(fileName, fileToUpload, {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profileResp.ok) {
        ...
        // ROLLBACK FRONTEND: Eliminar las fotos recién subidas para evitar huérfanos
        if (uploadedDocs.length > 0) {
            const filesToRemove = uploadedDocs.map((doc: any) => doc.fileName);
            await supabase.storage.from('documentos_choferes').remove(filesToRemove);
        }
        
        // ROLLBACK AUTH USER si falló el perfil (para evitar auth users sin registro)
        // NOTA: Requiere edge function/admin role, pero si falló la API, al menos las fotos están seguras.
        ...
        <form onSubmit={handleRegister} className="space-y-6">
          ...
          {step === 4 && (
            <div className="space-y-4 animate-in slide-in-from-right-4 fade-in">
               ...
               <div className="pt-2">
                 <FileUploader label="Certificado de Antecedentes Penales (Opcional)" file={antecedentesFile} setFile={setAntecedentesFile} />
                 <p className="text-xs text-zinc-500 mt-2 text-center">Podés completar este documento más adelante.</p>
               </div>
            </div>
          )}
          ...
          <div className="pt-4 border-t border-zinc-800/80 flex gap-3">
             ...
             {step < 5 ? (
                ...
                <button type="submit" disabled={loading || isOptimizing} className="flex-1 py-4 bg-[#0D6EFD] hover:bg-blue-500 text-white font-bold rounded-xl shadow-[0_0_20px_rgba(13,110,253,0.3)] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                    {loading || isOptimizing ? (
                        <>
                           <Loader2 className="animate-spin" size={20} />
                           {isOptimizing ? "Optimizando..." : "Enviando..."}
                        </>
                    ) : "Enviar Solicitud"}
                </button>
             )}
