  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
        ...
        <div className="xl:col-span-7 flex flex-col gap-4 sm:gap-6">
            ...
            <div className={`relative overflow-hidden p-5 sm:p-6 rounded-3xl border transition-all duration-500 ${isDebt ? 'bg-red-500/5 border-red-500/20 shadow-[0_10px_30px_-10px_rgba(239,68,68,0.15)]' : 'bg-blue-600/10 border-blue-500/30 shadow-[0_10px_30px_-10px_rgba(37,99,235,0.15)]'}`}>
            ...
            <div className="bg-zinc-900/60 border border-white/5 backdrop-blur-md rounded-3xl p-4 sm:p-6 shadow-xl w-full">
                <div className="flex items-center gap-3 mb-5 sm:mb-6">
                   <img src={logoUbi} alt="Traslados UBI" className="w-12 h-12 object-contain rounded-xl bg-white/5 p-1 border border-white/10" />
                ...
                <div className="bg-zinc-950/80 p-3 sm:p-5 rounded-2xl border border-blue-500/20 mb-6 flex flex-col gap-4">
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                        ...
                        <button 
                            ...
                            className="bg-[#009EE3]/15 hover:bg-[#009EE3]/25 text-[#009EE3] text-xs sm:text-sm font-black px-5 py-3 rounded-xl flex items-center gap-2 border border-[#009EE3]/30 transition-all active:scale-95 w-full sm:w-auto justify-center whitespace-nowrap"
                        >
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-zinc-300 w-full">
                        <div className="bg-zinc-900/70 p-3.5 rounded-xl border border-white/5 flex flex-col justify-center min-w-0">
                            <span className="block text-[10px] sm:text-xs uppercase text-zinc-500 font-black mb-1">Banco</span>
                            <span className="font-bold text-white text-sm sm:text-base truncate">Banco Provincia del Chaco</span>
                        </div>
                        <div className="bg-zinc-900/70 p-3.5 rounded-xl border border-white/5 flex flex-col justify-center min-w-0">
                            <span className="block text-[10px] sm:text-xs uppercase text-zinc-500 font-black mb-1">Titular</span>
                            <span className="font-bold text-white text-sm sm:text-base truncate">TRASLADOS UBI S.R.L.</span>
                        </div>
                        
                        <div className="bg-zinc-900/70 p-3.5 rounded-xl border border-white/5 flex justify-between items-center group min-w-0 sm:col-span-1">
                            <div className="min-w-0 pr-2">
                                <span className="block text-[10px] sm:text-xs uppercase text-zinc-500 font-black mb-1">CBU</span>
                                <span className="font-bold text-white text-[13px] sm:text-base tracking-tight break-all block">3110030211000012345678</span>
                            </div>
                            <button 
                                type="button"
                                onClick={() => copyToClipboard("3110030211000012345678", "cbu")}
                                className={`shrink-0 flex items-center justify-center p-2.5 rounded-lg transition-all active:scale-95 ${copiedField === 'cbu' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-zinc-800/80 text-zinc-400 hover:text-white hover:bg-zinc-700'}`}
                                title="Copiar CBU"
                            >
                        ...
                        <div className="bg-zinc-900/70 p-3.5 rounded-xl border border-white/5 flex justify-between items-center group min-w-0 sm:col-span-1">
                            <div className="min-w-0 pr-2">
                                <span className="block text-[10px] sm:text-xs uppercase text-zinc-500 font-black mb-1">Alias</span>
                                <span className="font-bold text-white text-[13px] sm:text-base tracking-tight break-all block">UBI.TRASLADOS.OFICIAL</span>
                            </div>
                            <button 
                                type="button"
                                onClick={() => copyToClipboard("UBI.TRASLADOS.OFICIAL", "alias")}
                                className={`shrink-0 flex items-center justify-center p-2.5 rounded-lg transition-all active:scale-95 ${copiedField === 'alias' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-zinc-800/80 text-zinc-400 hover:text-white hover:bg-zinc-700'}`}
                                title="Copiar Alias"
                            >
                ...
                <form onSubmit={handleUploadPago} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5">
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full bg-zinc-950/50 border border-zinc-700/50 p-4 text-lg rounded-xl text-white font-bold focus:ring-2 focus:ring-blue-500 outline-none transition-all placeholder:text-zinc-600"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                required
                                className="w-full bg-zinc-950/50 border border-zinc-700/50 p-3 rounded-xl text-zinc-400 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-xs file:font-bold file:uppercase file:bg-blue-500/10 file:text-blue-400 hover:file:bg-blue-500/20 transition-all cursor-pointer h-[60px]"
                            />
                        </div>
                    </div>
                    ...
                    <button 
                        type="submit"
                        disabled={payLoading}
                        className="w-full bg-blue-600 hover:bg-blue-500 text-white font-black py-4 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20 active:scale-95 disabled:opacity-50 transition-all mt-4 text-base uppercase tracking-wider"
                    >
                        {payLoading ? <Loader2 className="animate-spin" size={20} /> : <><Upload size={20}/> ENVIAR PAGO Y COMPROBANTE</>}
                    </button>
                </form>
            </div>
        </div>
        ...
        <div className="xl:col-span-5 flex h-[450px] xl:h-auto">
            <div className="bg-zinc-900/60 border border-white/5 backdrop-blur-md rounded-3xl p-5 sm:p-6 shadow-xl w-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto pr-1 space-y-4 custom-scrollbar">
                    {pagos.length === 0 ? (
                        ...
                        return (
                            <div key={p.id} className="group bg-zinc-950/50 border border-white/5 p-4 sm:p-5 rounded-2xl flex flex-col gap-3 hover:bg-zinc-800/50 transition-all">
