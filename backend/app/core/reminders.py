...
async def procesar_y_enviar_recordatorios():
    ...
    try:
        res = supabase.table("choferes").select(
            "id, saldo, usuarios(nombre, telefono)"
    ...
    for chofer in choferes_morosos:
        usuario = chofer.get("usuarios") or {}
        nombre = usuario.get("nombre", "Chofer").split(" ")[0] if usuario.get("nombre") else "Chofer"
        telefono = usuario.get("telefono", "") if usuario.get("telefono") else ""
        saldo_deuda = abs(chofer.get("saldo", 0))
        instancia_ai = "viajesnea"
