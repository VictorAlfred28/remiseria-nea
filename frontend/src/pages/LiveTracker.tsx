...
import { Car, Loader2, MapPin, Navigation, Info, ShieldCheck, CheckCircle2, ArrowLeft } from "lucide-react";
  ...
  return (
    <div className="h-screen w-full flex flex-col bg-zinc-950 relative overflow-hidden font-sans text-white">
                ...
                <div className="flex border-b border-white/5 pb-3 mb-3 items-center justify-between">
                    <div className="flex items-center gap-3">
                        <button 
                          ...
                          className="bg-white/10 hover:bg-white/20 p-2.5 rounded-full transition-all text-white pointer-events-auto shadow-lg active:scale-95"
                          title="Volver"
                        >
                            <ArrowLeft size={20} />
                        </button>
        ...
        <div className="flex-1 w-full relative z-0">
            {(liveCoords || data.origen?.lat) ? (
                <MapContainer 
                  center={liveCoords ? [liveCoords.lat, liveCoords.lng] : [data.origen.lat, data.origen.lng]} 
                  zoom={15} 
                ...
                >
                    ...
                    {liveCoords && <RecenterMap lat={liveCoords.lat} lng={liveCoords.lng} />}
                ...
                <div className="h-full w-full bg-zinc-900 flex flex-col items-center justify-center">
                    ...
                    <p className="text-zinc-400 font-medium tracking-wide">Iniciando mapa...</p>
                </div>
            )}
            
            {/* Overlay temporal cuando no hay GPS del chofer */}
            {!liveCoords && data.origen?.lat && (
                <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-[400] bg-black/60 backdrop-blur-md px-4 py-2 rounded-xl border border-white/10 flex items-center gap-3">
                   <Loader2 className="animate-spin text-blue-400" size={16} />
                   <p className="text-sm font-medium text-white shadow-sm">Obteniendo GPS del chofer...</p>
                </div>
            )}
        </div>
