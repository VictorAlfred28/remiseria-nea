  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (active) {
          ...
          setBaseFare(active.base_fare || 0);
          setFractionPrice(active.per_fraction_price || 0);
          setFractionKm(active.fraction_km || 0.10);
      }
    ...
    <div className="flex flex-col gap-10 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <form onSubmit={handleCreate} className="flex flex-col gap-6 bg-zinc-900 border border-zinc-800 p-6 rounded-2xl shadow-xl">
            ...
            <button type="submit" disabled={loading} className="w-full max-w-full min-h-[52px] px-4 py-3 text-base font-semibold text-center overflow-hidden whitespace-normal break-words shrink-0 bg-blue-600 hover:bg-blue-500 text-white rounded-xl transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
                {loading ? <Loader2 className="animate-spin" /> : "Guardar Nueva Tarifa"}
            </button>
        </form>
      </div>

      <div className="relative flex flex-col p-3 sm:p-4 border-t lg:border-t-0 lg:border-l border-zinc-800 mt-4 lg:mt-0">
          <div className="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
              <h3 className="text-xl font-bold text-white flex items-center gap-2"><History size={22} className="text-zinc-500 shrink-0"/> Historial de Tarifas</h3>
              <button onClick={() => window.open('/admin/print-tariff', '_blank')} className="w-full md:w-auto max-w-full min-h-[52px] px-4 py-3 text-base font-semibold text-center overflow-hidden whitespace-normal break-words shrink-0 bg-[#15803d] hover:bg-green-600 text-white rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-green-500/20 transition-all">
                  <Printer size={16} className="shrink-0" /> Imprimir A4
              </button>
          </div>
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-xl mt-4">
        ...
        <form onSubmit={handleCreateDest} className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-8 bg-zinc-950 p-4 rounded-xl border border-zinc-800 shadow-inner">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full">
                <select value={newDest.column_index} onChange={e=>setNewDest({...newDest, column_index: Number(e.target.value)})} className="flex-1 min-w-0 bg-zinc-900 border border-zinc-800 rounded-lg px-3 py-2 text-white outline-none text-sm cursor-pointer w-full">
                    <option value={1}>Col 1</option>
                    <option value={2}>Col 2</option>
                </select>
                <button type="submit" disabled={destLoading} className="flex-shrink-0 px-4 h-[38px] min-w-[100px] rounded-lg whitespace-nowrap bg-red-600 hover:bg-red-500 text-white font-bold transition-colors flex justify-center items-center gap-1 disabled:opacity-50 text-sm">
        ...
        <div className="overflow-x-auto w-full max-w-full">
            <div className="flex flex-col md:flex-row gap-4 min-w-[320px] max-h-[70vh] overflow-y-auto pr-2">
