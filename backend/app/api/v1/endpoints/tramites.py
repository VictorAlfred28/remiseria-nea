...
async def actualizar_tramite(
    ...
    # Verificar propiedad: el trámite debe pertenecer al usuario o a un vehículo suyo
    tramite = supabase.table("tramites").select("chofer_id, vehiculo_id, organizacion_id").eq("id", str(tramite_id)).execute()
    if not tramite.data:
        raise HTTPException(status_code=404, detail="Trámite no encontrado.")
    t = tramite.data[0]
    
    if rol not in ("admin", "superadmin"):
        es_propietario = (t.get("chofer_id") == user_id)
        if not es_propietario and t.get("vehiculo_id"):
            v_check = supabase.table("vehicles").select("id").eq("id", t["vehiculo_id"]).eq("titular_id", user_id).execute()
            es_propietario = bool(v_check.data)
        if not es_propietario:
            raise HTTPException(status_code=403, detail="No tienes permiso para modificar este trámite.")
...
async def eliminar_tramite(
    ...
    # Verificar propiedad
    tramite = supabase.table("tramites").select("chofer_id, vehiculo_id").eq("id", str(tramite_id)).execute()
    if not tramite.data:
        raise HTTPException(status_code=404, detail="Trámite no encontrado.")
    t = tramite.data[0]
    ...
    if rol not in ("admin", "superadmin"):
        ...
        if not es_propietario and t.get("vehiculo_id"):
            ...
            es_propietario = bool(v_check.data)
        if not es_propietario:
            raise HTTPException(status_code=403, detail="No tienes permiso para eliminar este trámite.")
